...
iprobit2 <- function(y, ..., kernel = "Canonical", maxit = 10000, stop.crit = 1e-5,
                     silent = FALSE, alpha0 = rnorm(1), lambda0 = abs(rnorm(2)),
                     w0 = rep(0, n)) {
  ...
  for (t in 1:(maxit - 1)) {
    ...
    # Update lambda[1] ---------------------------------------------------------
    P1 <- H1 + lambda[t, 2] * H12
    # P1sq <- P1 %*% P1
    # P1.H2 <- P1 %*% H2
    P1.H2 <- H1.H2 + lambda[t, 2] * t(H2.H12)
    S1 <- lambda[t, 2] * (P1.H2 + t(P1.H2))
    # ct1 <- sum(P1sq * W)
    ct1 <- sum(H1sq * W) + lambda[t, 2] ^ 2 * sum(H12sq * W) +
    ...
    # Update lambda[2] ---------------------------------------------------------
    P2 <- H2 + lambda[t + 1, 1] * H12
    # P2sq <- P2 %*% P2
    # P2.H1 <- P2 %*% H1
    P2.H1 <- t(H1.H2) + lambda[t + 1, 1] * t(H1.H12)
    S2 <- lambda[t + 1, 1] * (P2.H1 + t(P2.H1))
    # ct2 <- sum(P2sq * W)
    ct2 <- sum(H2sq * W) + lambda[t + 1, 1] ^ 2 * sum(H12sq * W) +
